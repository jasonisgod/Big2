...
def sampleFromNetwork():
    ...
    if a==enumerateOptions.passInd:
        ...
        idx = [str(i) for i in availableOptions].index(str(option))
        _onOptionSelect(idx, '')
        #print(idx, option, availableOptions)

def onOptionSelect(evt):
    w = evt.widget
    index = int(w.curselection()[0])
    value = w.get(index)
    _onOptionSelect(index, value)
    #print(type(evt), evt, 'onOptionSelect', index, value)
    
def _onOptionSelect(index, value):
    if value == "pass":
